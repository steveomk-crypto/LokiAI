...
COMPONENTS: dict[str, ComponentDef] = {
    ...
    'output_cycle': ComponentDef(
        id='output_cycle',
        name='Output Cycle',
        category='orchestration',
        kind='service',
        pid_file=SYSTEM_LOG_DIR / 'output_cycle.pid',
        log_path=SYSTEM_LOG_DIR / 'output_cycle.log',
        dependencies=['market_scanner'],
        notes='Slower output layer for broadcaster, Telegram, reports, and sidecars.',
        start_script='output_cycle_daemon.sh',
        inspect_target=SYSTEM_LOG_DIR / 'output_cycle.log',
        desired_default='on',
    ),
    'telegram_summary_cycle': ComponentDef(
        id='telegram_summary_cycle',
        name='Telegram Summary',
        category='orchestration',
        kind='service',
        pid_file=SYSTEM_LOG_DIR / 'telegram_summary.pid',
        log_path=SYSTEM_LOG_DIR / 'telegram_summary.log',
        dependencies=['telegram_sender'],
        notes='15-minute Telegram summary cadence.',
        start_script='telegram_summary_daemon.sh',
        inspect_target=SYSTEM_LOG_DIR / 'telegram_summary.log',
        desired_default='on',
    ),
    ...
    'market_broadcaster': ComponentDef(
        ...
        kind='job',
        log_path=SYSTEM_LOG_DIR / 'market_broadcaster.log',
        dependencies=['market_scanner'],
        notes='Builds posting/report output from scanner state.',
        inspect_target=SYSTEM_LOG_DIR / 'market_broadcaster.log',
        start_script='run_market_broadcaster.sh',
        start_label='Run',
        desired_default='auto',
    ),
    'telegram_sender': ComponentDef(
        ...
        category='output_plane',
        kind='job',
        log_path=SYSTEM_LOG_DIR / 'telegram_sender.log',
        dependencies=['market_broadcaster'],
        notes='Sends the latest status update to Telegram.',
        inspect_target=SYSTEM_LOG_DIR / 'telegram_sender.log',
        start_script='run_telegram_sender.sh',
        start_label='Run',
        desired_default='auto',
    ),
    'x_autoposter': ComponentDef(
        ...
        category='output_plane',
        kind='job',
        log_path=SYSTEM_LOG_DIR / 'x_autoposter.log',
        dependencies=['market_broadcaster'],
        notes='Draft, queue, and post the latest X updates when enabled.',
        inspect_target=WORKSPACE / 'x_posts',
        start_script='run_x_autoposter.sh',
        start_label='Post',
        desired_default='auto',
    ),
    'performance_analyzer': ComponentDef(
        ...
        category='output_plane',
        kind='job',
        log_path=SYSTEM_LOG_DIR / 'performance_analyzer.log',
        outputs=[WORKSPACE / 'performance_reports'],
        dependencies=['paper_trader_v2'],
        notes='Generates report artifacts.',
        inspect_target=SYSTEM_LOG_DIR / 'performance_analyzer.log',
        start_script='run_performance_analyzer.sh',
        start_label='Run',
        desired_default='auto',
    ),
